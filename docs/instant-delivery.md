...
## Webhook mode: seconds, not minutes
...
- On `checkout.session.completed` (and `async_payment_succeeded`), POSTs one
  ...
  it returns 502 — Stripe then retries the webhook on its own schedule, which
  is the relay's reliability layer.
...
| Signature check removed/broken | Worst case is dispatch spam → empty workflow runs burning Actions minutes. Fulfillment can't be forged — the workflow re-derives sales from Stripe's API and invites are idempotent. Still: the check is mandatory, and the tests pin it (`scripts/test/dispatch.test.js`). |
| Relay compromised | It holds two secrets: the webhook secret (can only make the relay believe Stripe called) and the GitHub token. It has no Stripe API key — it cannot read sales or move money. |
| GitHub token leaked | Fine-grained, single-repo. Contents R/W on the ops repo is real blast radius (it can push there) — scope it to *only* the ops repo, never an org-wide or classic PAT. Rotate: tokens expire ≤ 1 year. Tighter-token option: point the relay at `POST …/actions/workflows/fulfill.yml/dispatches` instead and the token needs only **Actions: R/W** (can't push) — you lose the typed event and the payload `ref`. |
...
| Relay down / Stripe webhook outage | Delivery degrades to the poll — exactly today's behavior. Stripe also retries failed webhook deliveries for days and emails you about a failing endpoint. |
...
No relay, no webhook — just a second, independent scheduler. A workflow in a
...
*only* the ops repo, **Actions: Read and write** — it can start runs, not
read code or secrets, not push) and variable `OPS_REPO`. Details and honest
...
- **Staying on the plain poll costs nothing** and remains the honest default:
  set the buyer's expectation at checkout ("within the hour — usually
  minutes") and beat it.
